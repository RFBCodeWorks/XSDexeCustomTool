...
namespace XSDCustomToolVSIX.Generate_Helpers
{
    ...
    /// </summary>
    internal class CodeGenerator_SupplementFile : CodeGenerator_Base
    {

        internal CodeGenerator_SupplementFile(ParsedFile parsedFile) : base(parsedFile) 
        {
        ...
        protected virtual void Generate_OutputClassComments(CodeTypeDeclaration @class, DiscoveredClass dClass)
        {
            ...
            @class.Comments.Add($"Partial Class {dClass.ClassName} generated by XSD.exe");
            @class.Comments.Add("</summary>");
        }
        ...
        protected virtual void Generate_OutputClassConstructors(CodeTypeDeclaration @class, DiscoveredClass dClass)
        {
            if (!dClass.HasConstructor)
            {
                ...
                Cstr.Comments.Add("<summary> Parameterless Constructor </summary>");
        ...
        protected virtual void Generate_ShouldSerializeRegion(CodeTypeDeclaration @class, DiscoveredClass dClass)
        {
            ...
            foreach (DiscoveredProperty dProp in dClass.ClassProperties.Where((DiscoveredProperty p) => p.IsSerializable))
            {
                CodeTypeMember mbr = ShouldSerializeProperty(dProp.Name);
                if (dClass.ClassProperties.First() == dProp)
                {
                    mbr.StartDirectives.Add(StartRegion("ShouldSerializeProperty"));
                    mbr.Comments.AddRange(GetComment_ShouldSerialize());
                }
                else if (dClass.ClassProperties.Last() == dProp)
        ...
        /// <returns></returns>
        protected virtual CodeMemberMethod ShouldSerializeProperty(string PropName)
        {
            CodeMemberMethod method = new CodeMemberMethod();
            method.Comments.Add($"<summary>Determine when the <see cref=\"{PropName}\"/> property is written to disk during XML Serialization</summary>", true);
        ...
        protected virtual CodeCommentStatementCollection GetComment_ShouldSerialize()
        {
            CodeCommentStatementCollection CommentBlock = new CodeCommentStatementCollection();
            CommentBlock.Add(NL);
            CommentBlock.Add($"ShouldSerialize is run  by the XML Serializer against properties to determine whether to write them to disk.{NL}");
            CommentBlock.Add($"The Default functionality (without this method) is to only serialize if the value is changed from the default.{NL}");
            CommentBlock.Add($"These methods override that functionality, allowing the programmer to decide when they are serialized.{NL}");
            CommentBlock.Add($"To restore original functionality (allowing Serializer to decide for each parameter), comment out these methods.{NL}");
            CommentBlock.Add(NL);
            return CommentBlock;
        }
